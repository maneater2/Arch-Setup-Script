...
user_password_prompt () {
    output 'Enter your user password (no echo):'
    ...
    output 'Confirm your user password (no echo):'
...
unpriv curl -s https://raw.githubusercontent.com/maneater2/Arch-Setup-Script/master/etc/sysconfig/chronyd | tee /mnt/etc/sysconfig/chronyd > /dev/null
...
unpriv curl -s https://raw.githubusercontent.com/maneater2/Arch-Setup-Script/master/etc/ssh/ssh_config.d/10-custom.conf | tee /mnt/etc/ssh/ssh_config.d/10-custom.conf > /dev/null
unpriv curl -s https://raw.githubusercontent.com/maneater2/Arch-Setup-Script/master/etc/ssh/sshd_config.d/10-custom.conf | tee /mnt/etc/ssh/sshd_config.d/10-custom.conf > /dev/null
...
if [ "${install_mode}" = 'server' ]; then
    unpriv curl -s https://raw.githubusercontent.com/maneater2/Arch-Setup-Script/master/etc/unbound/unbound.conf | tee /mnt/etc/unbound/unbound.conf > /dev/null
fi

## Setup Networking

if [ "${install_mode}" = 'desktop' ]; then
    unpriv curl -s https://raw.githubusercontent.com/maneater2/Arch-Setup-Script/master/etc/NetworkManager/conf.d/00-macrandomize.conf | tee /mnt/etc/NetworkManager/conf.d/00-macrandomize.conf > /dev/null
    unpriv curl -s https://raw.githubusercontent.com/maneater2/Arch-Setup-Script/master/etc/NetworkManager/conf.d/01-transient-hostname.conf | tee /mnt/etc/NetworkManager/conf.d/01-transient-hostname.conf > /dev/null
fi
